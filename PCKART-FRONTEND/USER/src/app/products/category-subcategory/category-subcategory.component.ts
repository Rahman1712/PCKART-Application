...
export class CategorySubcategoryComponent implements OnInit{
  ...
  public productsListImgs: ProductDTO<any>[] | undefined;
  ...
  public getAllProductsByCategoryName(categoryName: string): void {
    ...
    .pipe(
      map((x: ProductDTO<any>[] , i) => 
        x.map((product: ProductDTO<any>) => {
  ...
  productAddToCart(productDto: ProductDTO<any>){
    this.cartService.addToCart(productDto.productResponse, this.user!);
  }

  productAddToWish(productDto: ProductDTO<any>){

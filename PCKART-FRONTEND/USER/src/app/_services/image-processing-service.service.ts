...
export class ImageProcessingServiceService {
  ...
  public createMainImageToProdDto(productDto: ProductDTO<any>){
    const imgdataModel: any = productDto.imgdataModel;

    const imageBlob = this.dataURItoBlob(imgdataModel.image, imgdataModel.imgType);

    const imageFile = new File([imageBlob], imgdataModel.imgName, { type: imgdataModel.imgType })
  ...
  public createMainAndSubImagesToProdDto(productDto: ProductDTO<any>){
    const imgdataModelList: any[] = productDto.imgdataModel;

    const productSubImagesToFileHandle: FileHandle[] = [];

    for(let i=0; i<imgdataModelList.length; i++){
      const imageModelData = imgdataModelList[i];

      const imageBlob = this.dataURItoBlob(imageModelData.image, imageModelData.imgType);

      const imageFile = new File([imageBlob], imageModelData.imgName, { type: imageModelData.imgType })
  ...
  public createAllImagesToProdDto(productDto: ProductDTO<any>){
    const imgdataModelList: any[] = productDto.imgdataModel;
  ...
  public createImagesToPagination(prodPage: ProductPagination<ProductDTO<any>>){
